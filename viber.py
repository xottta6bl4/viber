...
class JobItilium:
    ...
    def get_state(self, environ, sender):
        ...
        try:
            req = requests.Request('POST', AddressApiItilium,
                                   headers=headers,
                                   data=data_to_send.encode('utf-8'), auth=HttpNtlmAuth(LoginItilium, PasswordItilium))
            prepped_requ = req.prepare()
            s = requests.Session()
            response = s.send(prepped_requ, verify=False)
            ...
            answer = Answer()
            answer.description = "1Ошибка соединения с Итилиум. Обратитесь к администратору."
            answer.status = False
            return answer
    ...
    def not_exist(self, sender):
        print_debug("not_exist(self, sender)")
        quote = "\""
        try:
            req = requests.Request('POST', AddressApiItilium,
                                                            data= """{
              ...
              auth=HttpNtlmAuth(LoginItilium, PasswordItilium))
            prepped_requ = req.prepare()
            s = requests.Session()
            response = s.send(prepped_requ, verify=False)
    ...
    def register(self, sender, message):
        ...
        try:
            ...
            req = requests.Request('POST', AddressApiItilium, data=json.dumps({
                                                                "data": {
                                                                "action": "register",
                                                                "sender": sender,
                                                                "phone":  text
    ...
    def sdregister_new_incident(self, message: str, sender: str):
    ...
    def daitoregister_new_incident(self, message: str, sender: str):
        print_debug("def register_new_incident")
        quote = "\""
        message = message
        data_to_send = """{
                           ...
                           "text":  """ + quote + message + quote + """,
                           "topic": "Отправлено с Viber",
                           "work": "701 Отдел мониторинга и диспетчеризации",
                           "type": "Запрос к ДЭиТО",
                           }
                        }"""
        print_value(data_to_send)
    ...
    def get_keyboard_class():
        return KeyboardMessage(min_api_version=4, keyboard=
        {
            ...
            "Buttons": [{
                "Columns": 3,
                "Rows": 1,
                "ActionBody": "_Itilium_bot_sd",
                "Text": "Департамент ИТ"
            }, {
                "Columns": 3,
                "Rows": 1,
                "ActionBody": "_Itilium_bot_701",
                "Text": "Департамент эксплуатации и ремонта"
            }, {
                "Columns": 6,
                "Rows": 1,
...
class JobMessage:
    ...
    def sdregister_incident(self, job_itilium: JobItilium, message: str, sender: str):
        print_debug("sdregister_incident")
        answer = job_itilium.sdregister_new_incident(message, sender)
        if answer.status:
    ...
    def daitoregister_incident(self, job_itilium: JobItilium, message: str, sender: str):
        print_debug("daitoregister_incident")
        answer = job_itilium.daitoregister_new_incident(message, sender)
        if answer.status:
            return [TextMessage(text="Зарегистрировано обращение:" + answer.result),
                    TemplatesKeyboards.get_keyboard_start_message()]
        else:
    ...
    def start_registration(self, sender: str):
        ...
        return [TextMessage(text="Выберите подразделение:"),
                            TemplatesKeyboards.get_keyboard_class()]

    def sdstart_registration(self, sender: str):
        print_debug("sdstart_registration")
        started_action = StartedAction("sdRegistration", "")
    ...
    def daitostart_registration(self, sender: str):
        print_debug("daitostart_registration")
        started_action = StartedAction("daitoRegistration", "")
        request_ok, description = SaveState(started_action,sender)
        if request_ok == False:
            return [TextMessage(text=description), TemplatesKeyboards.get_keyboard_start_message()]
        print_debug("add started action")
    ...
    def on_command_select(self, message: str, sender: str):
        ...
        if (message == '_Itilium_bot_sd'):
            return self.sdstart_registration(sender)
        elif (message == "_Itilium_bot_new_incident" ):
            return self.start_registration(sender)
        elif (message == '_Itilium_bot_701'):
            return self.daitostart_registration(sender)
        elif (message == "_Itilium_bot_Modify"):
            return self.start_itilium_modification(sender)
        elif (message == "_Itilium_bot_cancel_confirmation"):
            return [TextMessage(text="Регистрация отменина"), TemplatesKeyboards.get_keyboard_start_message()]
    ...
    def sdcontinue_registration(self, message, sender: str):
        ...
        if (command == "_Itilium_bot_cancel"):
            ...
            return self.sdregister_incident(job_itilium, command, sender)

    def daitocontinue_registration(self, message, sender: str):
        print_debug("continue_registration")
        job_itilium = JobItilium()
        request_ok, description = self.remove_started_action(sender)
        if request_ok == False:
        ...
        if (command == "_Itilium_bot_cancel"):
            ...
            return self.daitoregister_incident(job_itilium, command, sender)
    ...
    def continue_confirmed_select_rating(self, message, sender: str, started_action: StartedAction):
        ...
        print_value("nacalo" + reference_incident)
        request_ok, description = self.remove_started_action(sender)
        if request_ok == False:
            return [TextMessage(text=description), TemplatesKeyboards.get_keyboard_start_message()]
        if (command == "_Itilium_bot_Confirm_rating_cancel"):
            return [TextMessage(text="Подтверждение не выполнено."), TemplatesKeyboards.get_keyboard_start_message()]
        elif command == "_Itilium_bot_Confirm":
            ...
            if (answer == False):
                ...
                return [TextMessage(text=answer.result), TemplatesKeyboards.get_keyboard_start_message()]
        elif command == "_Itilium_bot_cancel_confirmation":
            return [TextMessage(text="Подтверждение не выполнено."), TemplatesKeyboards.get_keyboard_start_message()]
    ...
    def continue_started_process(self, message, sender: str):
        ...
        if (started_action.name == "sdRegistration"):
            return self.sdcontinue_registration(message, sender)
        elif (started_action.name == "daitoRegistration"):
            return self.daitocontinue_registration(message, sender)
...
def incoming():
    ...
    y = f.read()
    f.close()
    x = viber_request.message.text
    print_debug("значение файла" + y)
    if y == x:
        pass
        print_debug("сработало повторение")

    elif isinstance(viber_request, ViberMessageRequest):
        f = open(c, "w")
        f.write(x)
        f.close()
        print_debug("пошла движуха")
        ...
        if isReg:
            ...
            print_debug("сработало иф" + x)

        else:
            print_debug("сработало елсе" + x)
